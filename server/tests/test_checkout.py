...
def test_checkout_creates_record(app, patron):
    co = checkout_service.checkout_item(card="1234567890", item_input="9999999999", category="book")
    assert co.action == "checkout"
    assert co.weeks == 3
    assert co.due_date is not None


def test_checkout_2w_prefix(app, patron):
    co = checkout_service.checkout_item(card="1234567890", item_input="2W9999999999")
    assert co.weeks == 2


def test_checkout_dedupes_active(app, patron):
    checkout_service.checkout_item(card="1234567890", item_input="9999999999")
...
def test_return_clears_active(app, patron):
    ...
    co = checkout_service.checkout_item(card="1234567890", item_input="9999999999")
    assert co.action == "checkout"
...
def test_renew_extends_due_date(app, patron):
    ...
    renewed = checkout_service.renew_item("9999999999", weeks=3)
    assert renewed.due_date > original_due


def test_summary_counts(app, patron):
    checkout_service.checkout_item(card="1234567890", item_input="1111111111")
    checkout_service.checkout_item(card="1234567890", item_input="2222222222")
